import 'package:bid_right_mobile/models/winnings.dart';
import 'package:bid_right_mobile/ui/winnings/components/card_section.dart';
import 'package:bid_right_mobile/ui/winnings/components/labeled_info.dart';
import 'package:flutter/material.dart';

class PropertyInfoSection extends StatelessWidget {
  final PastWinningDetail winning;

  const PropertyInfoSection({Key? key, required this.winning})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    if (winning.properties.isEmpty) {
      return const SizedBox.shrink();
    }

    return MultiCardSection(
      label: 'Property Information',
      children: [
        for (final property in winning.properties)
          Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              Padding(
                padding: const EdgeInsets.all(4),
                child: LabeledInfo(
                  label: 'Address',
                  text: property.address.toString().toUpperCase(),
                ),
              )
              // TODO: details
            ],
          )
      ],
    );
  }
}
